...
@app.route("/anna-home")
def anna_index():
    """Return the Anna Rules homepage."""
    return render_template("anna-home.html")   

@app.route("/get_plot")
def get_plot_by_anna():
    page_with_plot = str()
    """Endpoint to get plot from file"""
    with open("./static/plt.html" , "r") as f:
        page_with_plot = f.read()
        f.close()
    return page_with_plot
